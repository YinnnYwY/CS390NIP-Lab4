...
OUTPUT_DIR = "./outputs/"+ "conv" + str(disc_updates) +":" + str(gen_updates)+OUTPUT_NAME
...
def buildDiscriminator():
    ...
    if DATASET == "cifar_10":
        model.add(Conv2D(64, (5,5), padding='same', input_shape=IMAGE_SHAPE))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Conv2D(128, (5,5), strides=(2,2), padding='same'))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Conv2D(128, (5,5), strides=(2,2), padding='same'))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Conv2D(256, (7,7), strides=(2,2), padding='same'))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Flatten())
...
def buildGenerator():
    ...
    if DATASET == "cifar_10":
        n_nodes = 256 * 4 * 4
        model.add(Dense(n_nodes, input_shape=(NOISE_SIZE,)))
        model.add(LeakyReLU(alpha=0.2))
        ...
        model.add(Dense(256 * 7 * 7, input_shape = (NOISE_SIZE,)))
        model.add(LeakyReLU(alpha = 0.2))
        model.add(BatchNormalization(momentum = 0.8))
        ...
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum = 0.8))
        model.add(Conv2D(1, (3,3), strides=(2,2), activation='tanh', padding='same'))
...
#     print(adversary(generator(noise)).shape)
    ...
    for epoch in range(epochs):
        ...
        losses['D_losses'].append(advLoss[0])
        losses['G_losses'].append(genLoss/gen_updates)
        ...
        if loggingInterval > 0 and epoch % loggingInterval == 0:
            ...
            plot_losses(losses, path=OUTPUT_DIR + "/" + str(epoch) + "loss.png")

    return (generator, adversary, gan, losses)
...
def main():
    ...
    (generator, adv, gan, losses) = buildGAN(data, epochs = 60000, loggingInterval = 1000)
    plot_losses(losses, path=OUTPUT_DIR + "/" + "loss.png")
